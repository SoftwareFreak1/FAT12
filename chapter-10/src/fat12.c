...
/* Copy a caller's volume label into the fixed 11-byte BPB field,
   right-padding with spaces. The caller does not have to pre-pad. */
static void copy_volume_label(char volume_label[11], const char* label)
{
    size_t len = strlen(label);
    if (len > 11) len = 11;
    memcpy(volume_label, label, len);
    memset(volume_label + len, ' ', 11 - len);
}

static BootSector format_boot_sector(
    BlockDevice* device,
    FormatParams params,
    uint32_t total_sectors,
...
)
{
    ...
    copy_volume_label(bs.extended_bpb.volume_label, params.volume_label);
...
{
    /* Build the first FAT sector — all copies are identical */
    uint8_t sector[bs.bpb.bytes_per_sector];
    ...
    for (uint32_t fat = 0; fat < bs.bpb.num_fats; fat++)
    {
        uint32_t lba = bs.bpb.reserved_sector_count +
                       (fat * bs.bpb.fat_size_16);

        block_device_write(device, lba, 1, sector);

        if (bs.bpb.fat_size_16 > 1)
        {
            uint32_t zero_count = bs.bpb.fat_size_16 - 1;
            uint64_t zero_bytes = zero_count * bs.bpb.bytes_per_sector;
            uint8_t* zeros = (uint8_t*)malloc(zero_bytes);
            memset(zeros, 0, zero_bytes);
            block_device_write(device, lba + 1, zero_count, zeros);
            free(zeros);
        }
    }
}
...
static void format_root_dir(BlockDevice* device, BootSector* bs)
{
    ...
    memcpy(label->name, bs->extended_bpb.volume_label, 8);
    memcpy(label->ext, bs->extended_bpb.volume_label + 8, 3);
...
{
    /* Validate disk size */
    uint64_t total_sectors = block_device_sector_count(device);
    if (total_sectors < 50) return -1;

    /* Compute geometry */
    uint32_t bytes_per_sector = block_device_sector_size(device);
    const uint16_t ROOT_ENTRY_COUNT = 512;
    ...
    uint16_t sectors_per_fat = compute_sectors_per_fat(
        (uint32_t)total_sectors, 2, root_dir_sectors, bytes_per_sector);

    BootSector bs = format_boot_sector(device, params, (uint32_t)total_sectors, sectors_per_fat, bytes_per_sector);
