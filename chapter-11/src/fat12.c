...
)
{
    ...
    if ((src_entry->attr & 0x10) && !same_parent)
    {
...
{
    uint64_t total_sectors = block_device_block_count(disk);

    uint32_t root_dir_sectors =
        ((512 * 32) + (512 - 1)) / 512;

    uint16_t sectors_per_fat = compute_sectors_per_fat(
        (uint32_t)total_sectors, 2, root_dir_sectors);
