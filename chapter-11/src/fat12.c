    ...
    for (uint32_t fat = 0; fat < bs.bpb.num_fats; fat++)
    {
        ...
        if (bs.bpb.fat_size_16 > 1)
        {
            ...
            if (zeros != NULL)
            {
                ...
                block_device_write(disk, lba + 1, zero_count, zeros);
                free(zeros);
            }
        }
    }
...
static void format_root_dir(BlockDevice* disk, BootSector* bs)
{
    uint32_t count = root_dir_sector_count(*bs);
    ...
    block_device_write(disk, calculate_root_dir_lba(*bs), count, zeros);
    free(zeros);
}

int fat12_format(BlockDevice* disk)
{
    uint64_t total_sectors = block_device_sector_count(disk);
    if (total_sectors < 50) return -1;

    uint32_t bytes_per_sector = block_device_sector_size(disk);
    ...
    BootSector bs = format_boot_sector(disk, (uint32_t)total_sectors, sectors_per_fat, bytes_per_sector);
    format_fats(disk, bs);
    format_root_dir(disk, &bs);
