...
static void free_cluster_chain(uint8_t* fat, uint16_t cluster)
{
    while (cluster >= 0x002 && cluster < 0xFF0)
    {
        uint16_t next = read_fat12_entry(fat, cluster);
        DBG_PRINT("[ fat12 ] free cluster %u%s\n",
                  cluster, next >= 0xFF0 ? " (last)" : "");
...
)
{
    ...
    while (cluster >= 0x002 && cluster < 0xFF0 && remaining > 0)
    {
        uint32_t lba = ((cluster - 2) * bs.bpb.sectors_per_cluster)
                       + data_start;
        DBG_PRINT(
            "[ fat12 ] write: cluster %u -> LBA %u -> data offset %u\n",
            cluster, lba, (cluster - 2) * cluster_bytes);
...
int fat12_remove(BlockDevice* disk, const char* path)
{
    ...
    if (entry == NULL || (entry->attr & FAT12_ATTR_DIRECTORY))
    {
        free(entries);
        free(fat);
        return -1;
    ...
    if (entry->first_cluster >= 0x002)
        free_cluster_chain(fat, entry->first_cluster);
...
int fat12_rmdir(BlockDevice* disk, const char* path)
{
    ...
    DirectoryEntry* entry = find_by_name_in_entries(entries, count, dirname);
    if (entry == NULL || !(entry->attr & FAT12_ATTR_DIRECTORY))
    {
        free(entries);
        free(fat);
...
)
{
    ...
    if (find_by_name_in_entries(dst_entries, dst_count, dst_name) != NULL)
    {
        free(dst_entries);
        free(src_entries);
        free(fat);
    ...
    if (dst_slot < 0)
    {
        if (!same_parent) free(dst_entries);
        free(src_entries);
        free(fat);
        return -1;
    }
    ...
    memcpy(&dst_entries[dst_slot], src_entry,
           sizeof(DirectoryEntry));

    set_entry_name(&dst_entries[dst_slot], dst_name);

    dst_entries[dst_slot].create_date = 0x4D55;
    dst_entries[dst_slot].create_time = 0x7258;
    dst_entries[dst_slot].last_write_date = 0x4D55;
    dst_entries[dst_slot].last_write_time = 0x7258;
    dst_entries[dst_slot].last_access_date = 0x4D55;
...
static void write_zero_sectors(
    BlockDevice* disk,
    uint64_t start_lba,
    uint64_t count
)
{
    ...
    {
        block_device_write(disk, start_lba + i, 1, zero);
    }
}
...
)
{
    ...
    for (sf = 1; sf < 255; sf++)
    {
        uint32_t data_sectors =
            total_sectors - 1 - (2 * sf) - root_dir_sectors;
        uint32_t total_clusters =
            data_sectors / sectors_per_cluster;
        uint32_t fat_capacity =
            (sf * 512 * 8) / 12;

        if (total_clusters + 2 <= fat_capacity)
            break;
    }
...
static void write_fat_tables(
    BlockDevice* disk,
    BootSector bs,
    uint16_t sectors_per_fat
)
{
    for (uint32_t fat = 0; fat < bs.bpb.num_fats; fat++)
    {
        uint8_t sector[512];
        memset(sector, 0, sizeof(sector));

        sector[0] = bs.bpb.media;
        sector[1] = 0xFF;
        sector[2] = 0xFF;
        ...
        block_device_write(disk, lba, 1, sector);

        memset(sector, 0, sizeof(sector));

        for (uint32_t i = 1; i < sectors_per_fat; i++)
        {
            block_device_write(disk, lba + i, 1, sector);
        }
    }
}
...
{
    uint64_t total_sectors = block_device_sector_count(disk);
    ...
    uint32_t root_dir_sectors =
        ((bs.bpb.root_entry_count * 32) +
         (bs.bpb.bytes_per_sector - 1)) /
        bs.bpb.bytes_per_sector;

    uint16_t sectors_per_fat = compute_sectors_per_fat(
        (uint32_t)total_sectors,
        bs.bpb.sectors_per_cluster,
        root_dir_sectors);

    write_zero_sectors(disk, 0, total_sectors);
    ...
    else
    {
        ...
        bs.bpb.total_sectors_32 = (uint32_t)total_sectors;
    }

    bs.bpb.media = 0xF8;
    bs.bpb.fat_size_16 = sectors_per_fat;
    ...
    bs.extended_bpb.volume_id = 0x12345678;

    memcpy(bs.extended_bpb.volume_label, "NO NAME    ", 11);
    memcpy(bs.extended_bpb.file_system_type, "FAT12   ", 8);

    bs.signature = 0xAA55;

    block_device_write(disk, 0, 1, &bs);

    write_fat_tables(disk, bs, sectors_per_fat);
}
